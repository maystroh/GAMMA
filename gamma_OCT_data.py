...
def main():
    ...
    parser.add_argument('--height_fundus', type=int, default=356, help='image height after resizing')
    parser.add_argument('--width_fundus', type=int, default=356, help='image width after resizing')
    ...
    if 'classification' in args.gamma_task:
        ...
        for patient in glob.glob(os.path.join(multi_modal_dirs, '*')):
            ...
            # resizing images
            patient_image_dir = os.path.join(patient, patient_name + '_cropped/')
            if not os.path.exists(patient_image_dir):
                os.makedirs(patient_image_dir)

            for ind, bscan in enumerate(bscans):
                print(bscan)
                image_name = bscan.split("/")[-1]
                image_name = image_name.split(".")[0]

                image_bscan = cv2.imread(bscan)
                bottom = int(image_bscan.shape[0] * 0.7)

                roi = image_bscan[0:bottom, :]
                cv2.imwrite(patient_image_dir + image_name + '.png', roi)
