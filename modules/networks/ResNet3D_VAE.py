...
class ResNet3dVAE(BaseModel):
    ...
    def __init__(self, in_channels=2, classes=4, max_conv_channels=256, dim=(64, 64, 64)):
        ...
        # nb_features = 256  # 224 x 224 x 24
        nb_features = 256  # 224 x 224 x 32
        # nb_features = 128  # 224 x 224 x 64
        self.fc_linear = nn.Sequential(nn.Linear(nb_features, classes))

    def forward(self, x):
        x1, x2, x3, x4, x_out = self.encoder(x)

        # y = self.decoder(x1, x2, x3, x4)
        # vae_out, mu, logvar = self.vae(x4)
        # return y, vae_out, mu, logvar

        fc1 = nn.functional.adaptive_avg_pool3d(x_out, (1, 1, 1))
        fc1 = self.fc_linear(fc1.squeeze())
        predict = torch.softmax(fc1)
        return predict
